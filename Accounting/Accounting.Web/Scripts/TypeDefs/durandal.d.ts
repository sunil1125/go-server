...
/// <reference path="../../App/services/models/TypeDefs/CommonModels.d.ts" />
...
interface DurandalAppModule extends DurandalEventSupport<DurandalAppModule> {
	...
	*/
	 ...
	 * @returns {Promise} A promise that resolves when the dialog is closed and returns any data passed at the time of closing.
	*/
	showDialog(obj: any, activationData?: IMessageBoxOption, context?: string): JQueryPromise<any>;
	showDialog(obj: any, activationData?: IWidgetPopupOption, context?: string): JQueryPromise<any>;
...
interface DurandalRouteConfigurationSettings {
	dashboard?: boolean;
	VendorBill?: boolean;
	MyAccount?: boolean;
	Reports?: boolean;
	...
	Rebill?: boolean;
	/**
	Member to set the content area
	**/
	isDenimBackground?: boolean;
	/**
	* Member to set main menu flag.
	*/
	isMainMenu?: boolean;
	/**
	* Logical resource name from acgsec.
	*/
	resourceName?: string;
	/**
	* Required permission type[from acgsec]
	*/
	rqdPermission?: number;
}
...
interface DurandalRouterBase<T> extends DurandalEventSupport<T> {
	...
	*/
	...
	/**
	 * An observable surfacing the active routing instruction that is currently being active/processed or has recently finished activate/processing.
	 * The instruction object has `config`, `fragment`, `queryString`, `params` and `queryParams` properties.
	*/
	activeRouteInstruction: KnockoutObservable<DurandalRouteInstruction>;
