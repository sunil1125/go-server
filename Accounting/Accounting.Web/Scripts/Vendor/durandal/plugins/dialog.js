...
define(['durandal/system', 'durandal/app', 'durandal/composition', 'durandal/activator', 'durandal/viewEngine', 'jquery', 'knockout'], function (system, app, composition, activator, viewEngine, $, ko) {
    ...
    dialog.addContext('default', {
        ...
        addHost: function(theDialog) {
            ...
            var blockout = $('<div class="modalBlockout"></div>')
                .css({ 'z-index': dialog.getNextZIndex(), 'opacity': this.blockoutOpacity, 'background': "white" })
                .appendTo(body);
    ...
    ///This context is added for the 'slideDown from top' effect to be applied to all the dialogs - common context
    dialog.addContext('slideDown', {
        blockoutOpacity: .85,
        removeDelay: 350,
        /**
         * In this function, you are expected to add a DOM element to the tree which will serve as the "host" for the modal's composed view. You must add a property called host to the modalWindow object which references the dom element. It is this host which is passed to the composition module.
        ...
        addHost: function (theDialog) {
            ...
            var host = $('<div class="modalHost" style="top:0"></div>')
                .css({ 'z-index': dialog.getNextZIndex() })
                .appendTo(body);
         ...
         */
        removeHost: function (theDialog) {
            jQuery(theDialog.host).animate({ top: -120 }, 600);
            //$(theDialog.host).css('opacity', 0);
            $(theDialog.blockout).css('opacity', 0);
        ...
        compositionComplete: function (child, parent, context) {
            ...
            $(theDialog.host).css('opacity', 1);
            jQuery(theDialog.host).animate({ top: 200 }, 500);
    ...
    ///This context is added for the 'zoomIn' effect to be applied to all the dialogs - common context
    dialog.addContext('zoomIn', {
        blockoutOpacity: .50,
        removeDelay: 350,
        /**
         * In this function, you are expected to add a DOM element to the tree which will serve as the "host" for the modal's composed view. You must add a property called host to the modalWindow object which references the dom element. It is this host which is passed to the composition module.
        ...
        addHost: function (theDialog) {
            var body = $('body');
            var blockout = $('<div class="modalBlockout"></div>')
                .css({ 'z-index': dialog.getNextZIndex(), 'opacity': this.blockoutOpacity, 'background-color': 'rgb(0, 0, 0)' })
                .appendTo(body);
         ...
         */
        removeHost: function (theDialog) {
            //jQuery(theDialog.host).fadeOut("slow");
        	jQuery(theDialog.host).animate({ 'z-index': '0', opacity: '0' }, 300);
        	$(theDialog.blockout).animate({ opacity: '0' }, 300);
        ...
        compositionComplete: function (child, parent, context) {
            ...
            // $(theDialog.host).css('opacity', 1);
            // jQuery(theDialog.host).fadeIn(3000);
            jQuery(theDialog.host).animate({ 'z-index': dialog.getNextZIndex(), opacity: '1' }, 300)
