...
(function ($) {
		...
		return this
				   .each(function () {
				   	...
				   	$($image).css({
				   		...
				   		'max-width': getData('image').w,
				   		'max-height': getData('image').h
				   	});
				   	$($image).attr('id', getData('image').id);
				   	...
				   	var aspectRatio_vp = getData('image').w / getData('image').h;
				   	var aspectratio_img = $($image).width() / $($image).height();
				   	getData('image').aspectratio = aspectratio_img;
				   	if (aspectRatio_vp <= getData('image').aspectratio) {
				   		$($image).css({ 'min-width': getData('image').w });
				   	}
				   	else {
				   		$($image).css({ 'min-height': getData('image').h });
				   	}
				   	$($image).css({
				   		'position': 'relative',
				   		'left': getData('image').posX,
				   		'top': (_self.height() - $image.height()) / 2,//to always align the image in the middle
				   		'max-width': getData('image').w,
				   		'max-height': getData('image').h
				   	});
				   	...
				   	$($image).draggable({
				   		...
				   		drag: function (event, ui) {
				   			...
				   			getData('image').posY = $options.image.y
				   			getData('image').posX = $options.image.x
				   	...
				   	function calculateTranslationAndRotation() {
				   		$(function () {
				   			...
				   			$($image).rotate(getData('image').rotation);
				   		});
				   	};
				   	...
				   	// Creating button rotation control.
				   	function createButtonRotationControl() {
				   		var buttonRotationContainer = $("<div/>").attr('id', 'buttonRotationContainer').addClass('buttonRotationContainer');

				   		// Creating new elements for the button control.
				   		var rigthButtonContainer = $('<div/>').attr('id', 'btnRotateRight').addClass('btnRotateRight');
				   		var rightRotateButtonImage = $('<img/>').attr('src', $options.rightButtonImage);
				   		var rightRotateButton = $("<span/>").attr('type', 'button').html('Right');
				   		rigthButtonContainer.append(rightRotateButtonImage).append(rightRotateButton);

				   		var leftButtonContainer = $('<div/>').attr('id', 'btnRotateLeft').addClass('btnRotateLeft');
				   		var leftRotateButtonImage = $('<img/>').attr('src', $options.leftButtonImage);
				   		var leftRotateButton = $("<span/>").attr('type', 'button').html('Left');
				   		leftButtonContainer.append(leftRotateButtonImage).append(leftRotateButton);

				   		buttonRotationContainer.append(rigthButtonContainer);
				   		buttonRotationContainer.append(leftButtonContainer);

				   		if ($options.expose.buttonControlElement != '') {
				   			$($options.expose.buttonControlElement).empty().append(
								buttonRotationContainer);
				   		} else {
				   			_self.append(buttonRotationContainer);
				   		}
				   		var value = Math.abs(360 - $options.image.rotation);
				   		var rotateBy = 0;
				   		//this function is to maintain the height of the image on rotation
				   		var onrotate = function ($cropzoom, rotation) {
				   			var $image = $("#" + getData('image').id);
				   			var aspectRatio_vp = getData('image').w / getData('image').h;

				   			if (rotation == 90 || rotation == 270) {
				   				if (aspectRatio_vp >= 1 / getData('image').aspectratio) {
				   					$($image).css({ 'min-width': getData('image').h });
				   					$($image).css({ 'min-height': "" });
				   				}
				   				else {
				   					$($image).css({ 'min-height': getData('image').w });
				   					$($image).css({ 'min-width': "" });
				   				}

				   				var width = Math.min($("img.ui-draggable").width(), $cropzoom.height());
				   				$("img.ui-draggable").css({
				   					'max-width': width,
				   					'max-height': getData('image').w,
				   				});
				   				var topValue = ($cropzoom.height() / 2 - $("img.ui-draggable").height() / 2);
				   				$("img.ui-draggable").css({
				   					'top': topValue,//to always align the image in the middle					
				   				});
				   				...
				   				$("img.ui-draggable").css({
				   					'top': ($cropzoom.height() - $("img.ui-draggable").height()) / 2,//to always align the image in the middle					
				   				});
				   			}

				   		}
				   		$('#btnRotateRight').click(function (event, ui) {

				   			rotateBy -= $options.rotationSteps;
				   			if (rotateBy < 0)
				   				rotateBy = 270;
				   			getData('image').rotation = (value == 360 ? Math
										.abs(360 - rotateBy)
										: Math.abs(rotateBy));
				   			calculateTranslationAndRotation();
				   			onrotate(_self, getData('image').rotation);
				   			if ($options.image.onRotate != null)
				   				$options.image.onRotate(_self,
									getData('image').rotation);
				   			resetSelector();
				   		});

				   		$('#btnRotateLeft').click(function (event, ui) {
				   			rotateBy += $options.rotationSteps;
				   			getData('image').rotation = (value == 360 ? Math
										.abs(360 - rotateBy)
										: Math.abs(rotateBy));
				   		...
				   		//to always align the selector on the center of the image after each rotation
				   		var resetSelector = function () {
				   			$selector = $('#' + _self[0].id + '_selector');
				   			var image = getData('image');

				   			if (image.rotation == 90 || image.rotation == 270) {
				   				imgWidth = $("#" + image.id).height();
				   				imgHeight = $("#" + image.id).width();
				   			}
				   			else {
				   				imgWidth = $("#" + image.id).width();
				   				imgHeight = $("#" + image.id).height();
				   			}

				   			var minTop = Math.abs((_self.height() - imgHeight) / 2);
				   			var minLeft = Math.abs((_self.width() - imgWidth) / 2);


				   			var selectorSizeWidth = Math.abs( 0.8 * imgWidth); //80% of image size is taken as selector size to give some gap around it
				   			var selectorSizeHeight = Math.abs( 0.8 * imgHeight);
				   			selectorSizeWidth = Math.min($options.selector.w, selectorSizeWidth);//selector size should be minimum of the either the default selector size or 80% of image size
				   			selectorSizeHeight = Math.min($options.selector.h, selectorSizeHeight);
							//To maintain the aspect ratio of selector
				   			if (selectorSizeWidth < selectorSizeHeight)
				   				selectorSizeHeight = Math.abs(selectorSizeWidth * $options.selector.h / $options.selector.w);
				   			else
				   				selectorSizeWidth = Math.abs(selectorSizeHeight * $options.selector.w / $options.selector.h);

				   			var sTop = minTop + Math.abs((imgHeight - selectorSizeHeight) / 2);
				   			var sLeft = minLeft + Math.abs((imgWidth - selectorSizeWidth) / 2);
				   			$selector.width(selectorSizeWidth);
				   			$selector.height(selectorSizeHeight);
				   			$selector.css("top", sTop);
				   			$selector.css("left", sLeft);

				   			//Repfresh overlay position
				   			getData('selector').w = selectorSizeWidth;
				   			getData('selector').h = selectorSizeHeight;
				   			getData('selector').x = sLeft;
				   			getData('selector').y = sTop;
				   			var ui = { position: { top: sTop, left: sLeft } };
				   	...
				   	function createSelector() {
				   		if ($options.selector.centered) {
				   			...
				   			sWidth = Math.min(Math.floor(0.7 * sSize), getData('selector').w);
				   			sHeight = Math.min(Math.floor(0.7 * sSize), getData('selector').h);
							//to maintain the aspect ratio of selector
				   			if (sWidth > sHeight)
				   				sHeight = Math.abs(sWidth * $options.selector.h / $options.selector.w);
				   			else
				   				sWidth = Math.abs(sHeight * $options.selector.w / $options.selector.h);
				   			getData('selector').w = Math.min(getData('selector').w, sWidth);
				   			getData('selector').h = Math.min(getData('selector').h, sHeight);
				   		...
				   		$selector = $('<div/>')
							...
							.css(
							{
								...
								'left': getData('selector').x
									+ 'px',
								/***
								*US11263:made border as 'solid' in place of 'dashed'*
								****/
								'border': '1px solid '
				   		...
				   		//check whether the selector goes out of the Content area and returns the min and max position of the selector
				   		var checkSelector = function ($selector) {
				   			...
				   			var minTop = Math.floor((_self.height() - imgHeight) / 2);
				   			var maxTop = minTop + imgHeight - 1 - $selector.height();
				   			var minLeft = Math.floor((_self.width() - imgWidth) / 2);
				   			var maxLeft = minLeft + imgWidth - 1 - $selector.width();

				   			return { minTop: minTop, maxTop: maxTop, minLeft: minLeft, maxLeft: maxLeft };
				   		}
				   		...
				   		$selector
							.draggable({
								...
								drag: function (event, ui) {
									...
									var pos = checkSelector($selector);
									if (ui.position.top >= pos.minTop && ui.position.top <= pos.maxTop && ui.position.left >= pos.minLeft && ui.position.left <= pos.maxLeft) {
										...
										//DE8196: this updates the position of selector within the image even if the cursor outside the cursor area.
										if (ui.position.top < pos.minTop)
											ui.position.top = pos.minTop;
										if (ui.position.top > pos.maxTop)
											ui.position.top = pos.maxTop;
										if (ui.position.left < pos.minLeft)
											ui.position.left = pos.minLeft;
										if (ui.position.left > pos.maxLeft)
											ui.position.left = pos.maxLeft;
										getData('selector').x = ui.position.left;
										getData('selector').y = ui.position.top;

										$selector.width(getData('selector').w);
										$selector.height(getData('selector').h);
										$selector.css("top", getData('selector').y);
										$selector.css("left", getData('selector').x);
										makeOverlayPositions(ui);
									}
				   		...
				   		$selector
							.resizable({
								...
								resize: function (event, ui) {
									...
									var getPosition = $selector.position();
									var pos = checkSelector($selector);
									if (getPosition.left <= 0 && getPosition.top <= 0) {
										$selector.width(getData('selector').w);
										$selector.height(getData('selector').h);
										$selector.css("top", getData('selector').y);
										$selector.css("left", getData('selector').x);
										...
										if (ui.position.top >= pos.minTop && ui.position.top <= pos.maxTop && ui.position.left >= pos.minLeft && ui.position.left <= pos.maxLeft) {
											...
											getData('selector').h = $selector
												.height();
											getData('selector').x = $selector.position().left;
											getData('selector').y = $selector.position().top;
											// update ovelay position
											makeOverlayPositions(ui);
										}
										else {
											//restict the selector from going out of image
											$selector.width(getData('selector').w);
											$selector.height(getData('selector').h);
											$selector.css("top", getData('selector').y);
											$selector.css("left", getData('selector').x);
				   	...
				   	$.fn.cropzoom.getParameters = function (_self, custom) {
				   		...
				   		var fixed_data = {
				   			...
				   			'imageW': $("#" + image.id).width(),//taking the current width of the image
				   			'imageH': $("#" + image.id).height(),//taking the current height of the image
				   			'imageSource': image.source,
				   			'selectorX': selector.x - (_self.width() - imgWidth) / 2,//calculationg the relative position of selector window
				   			'selectorY': selector.y - (_self.height() - imgHeight) / 2,//calculationg the relative position of selector window
	...
	$.fn.extend({
		...
		restore: function () {
			...
			if ($options.expose.buttonControlElement != "") {
				$($options.expose.buttonControlElement).empty();
			}
			if ($options.expose.elementMovement != "") {
