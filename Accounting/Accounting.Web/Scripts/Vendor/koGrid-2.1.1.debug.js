...
(function (window) {
	...
	***********************************************/
	window.kg.defaultCellTemplate = function () { return '<div data-bind="attr: { \'class\': \'kgCellText colt\' + $index(), title: $data.getProperty($parent)}, html: $data.getProperty($parent)"></div>'; };
	...
	***********************************************/
	window.kg.Column = function (config, grid) {
		...
		// Start : impplementation for the Non-Removable Columns - US6796
	...
	window.kg.EventProvider = function (grid) {
		...
		self.setDraggables = function () {
			if (!grid.config.jqueryUIDraggable) {
				...
				var calculatehelperPos = function (kogridContainer, ui) {
					...
					//get relative position of the handler wrt grid
				...
				grid.$root.find('.kgHeaderSortColumn').draggable({
					...
					drag: function (event, ui) {
						...
						// If we have the same column,or the coltomove or coltoreplace is non reorderable do nothing.
		...
		self.assignGridEventHandlers = function () {
			...
			$(window).resize(function () {
				...
				if (typeof(grid.config) !== "undefined" && grid.config.maintainColumnRatios) {
					grid.configureColumnWidths();
				}
			});
		};
	...
	***********************************************/
	window.kg.Grid = function (options) {
		var defaults = {
			...
			isHeaderHeightFixed: false, //US7008--set wether header should be fixed or stacked
		...
		self.configureColumnWidths = function () {
			...
			$.each(cols, function (i, col) {
				...
				if (isNaN(t) && columns[i].visible() == true) {
					...
					//so the width for columns with variable lenght gets correctly calculated
					if (!isNaN(t) && columns[i].visible() == true)
		...
		self.init = function () {
			...
			self.filteredData.subscribe(function () {
				...
				if (self.filteredData().length == (noOfSelectedData) && noOfSelectedData > 0)
		...
		self.sortData = function (col, direction) {
			...
			if (!self.config.useExternalSorting) {
				...
				if (self.config.fullDataObject && self.config.fullDataObject() && self.config.fullDataObject().length) {
					// sliceing the sorted data for the current page.
		...
		self.showMenu.subscribe(function (newValue) {
			...
			var hideMenu = function (obj) {
				if (obj.target.parentNode && obj.target.parentNode.classList && obj.target.parentNode.classList.length) {
					if (obj.target.parentNode.classList.contains("kgHeaderButton")) {
						return;
					}
					if (obj.target.parentNode.classList.contains("kgColMenu") || obj.target.className == "kgColMenu" ||
						obj.target.parentNode.classList.contains("kgColList") || obj.target.className == "kgColList" ||
						obj.target.parentNode.classList.contains("kgShowColumn") || obj.target.className == "kgShowColumn" ||
						obj.target.className == "kgMenuText") {
						return;
					}
				}

				// Do not close the settings Popup when user clicks on the Popup itself
				if (obj.target.className == "kgColMenu" ||
					obj.target.className == "btn" || // restore default button
					obj.target.className == "kgColList" ||
					obj.target.className == "kgShowColumn" ||
					obj.target.className == "kgMenuText") {
					return;
				}
		...
		window.kg.toggleSelection.subscribe(function (_newValue) {
			if (typeof(self.allSelected) !== "undefined" && _newValue != null) {
						...
						if (!condition.column) {
							for (var prop in item) {
								if (item.hasOwnProperty(prop)) {
									// get the coulmn setting for the current property
	...
	window.kg.SelectionService = function (grid) {
		...
		self.ChangeSelection = function (rowItem, evt) {
			...
			if (evt && evt.shiftKey && self.multi) {
				...
				if (window.kg.selectedCarrier) {
					rowValue = $.grep(grid.renderedRows(), function (e) {
						return e.selectedItems()[0].CarrierId === window.kg.selectedCarrier.CarrierId && e.selected();
					})[0];
				}

				if (rowValue !== undefined) {
					self.setSelection(rowValue, false);
				} else if (self.lastClickedRow && self.lastClickedRow != rowItem) {
			...
			//DE7603 --fix start
			//change master flag to true if all the filtered data is selected
			if (grid.filteredData().length == (grid.selectedItemCount()) && grid.selectedItemCount() > 0)
	...
	***********************************************/
	window.kg.sortService = {
		...
		alphaRE : /[^a-zA-Z]/g, // nasty regex for alphabet parsing
	...
	***********************************************/
	...
	window.kg.domUtilityService = {
		...
		UpdateGridLayout: function (grid) {
			...
			if (typeof (grid.$viewport) !== "undefined") {
			...
			$.each(cols, function (i, col) {
				css += "." + gridId + " .col" + i + " { width: " + col.width + "px; left: " + (sumWidth - 1) + "px; right: " + (trw - sumWidth - col.width) + "px; height: " + rowHeight + "px }" +
					"." + gridId + " .hcol" + i + " { max-width: " + (col.width - 1) + "px;min-width: " + (col.width - 1) + "px;}" + //US7008--seperate class for header column is added and 1px is removed from the header width for border
					   "." + gridId + " .colt" + i + " { width: " + (col.width - padding) + "px; }";
				sumWidth += col.width;
			});
