...
(function (window) {
	...
	***********************************************/
	...
	/***********************************************
       adding varibale for selection manipualtion
	 ***********************************************/
	window.kg.toggleSelection = ko.observable(null);
	...
	***********************************************/
	window.kg.defaultHeaderRowTemplate = function () { return '<div data-bind="foreach: visibleColumns"><div data-bind="kgHeaderCell: $data, attr: { \'class\': \'kgHeaderCell hcol\' + $index() }"></div></div>'; };

	/***********************************************
	* FILE: ..\src\templates\headerCellTemplate.html
	***********************************************/
	window.kg.defaultHeaderCellTemplate = function () { return '<div data-bind="style: { cursor : sortable() ? \'pointer\' : \'default\' }, click: sort, css: {\'kgSorted\': !noSortVisible }, attr: {\'class\': \'kgHeaderSortColumn \' + headerClass()}"><div class="kgSortButtonDown" data-bind="visible: showSortButtonDown"></div><div class="kgSortButtonUp" data-bind="visible: showSortButtonUp"></div><div data-bind="attr: { \'class\': \' kgHeaderText\' + \' kgHeaderFixed\' }, html: displayName"></div><div data-bind="visible: resizable, click: gripClick, mouseEvents: { mouseDown: gripOnMouseDown }" class="kgHeaderGrip" ></div></div>'; };
	...
	ko.bindingHandlers['koGrid'] = (function () {
		return {
			'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
				...
				options.isDirty = ko.computed(function () {
					return Utils.getDirtyItems(grid.config).length ? true : false;
				});
				options.setTrackChange = function () {
					grid.config.allowTrackChange = true;
					if (grid.config.allowTrackChange) {
						grid.config.data.extend({ trackChange: true });
					}
				}
	...
	***********************************************/
	window.kg.Column = function (config, grid) {
		...
		// Start : implementation for the Non-Removable Columns - US6796

		// Implementation for the columns which will not appear in Column Menu
		self._isListedForRemove = config.colDef.isListedForRemove;
		self.isListedForRemove = ko.computed(function () {
			if (self.displayName() == '\u2714' || (grid.config.checkBoxfilterField && self.displayName() == grid.config.checkBoxfilterField.toString()))
				return true;
			else if (self._isListedForRemove == undefined || typeof self._isListedForRemove == "object")
				return false;
			else
				return self._isListedForRemove;
		}, self);

		// Implementation for the columns which will are Non-Removable from grid
		self._isRemovable = config.colDef.isRemovable;
		self.isRemovable = ko.computed(function () {
			if (self._isRemovable == undefined || typeof self._isRemovable == "object")
				return true;
			else
				return self._isRemovable;
		}, self);

		// Implementation for the columns which are Custom Header Column in the grid
		self._isCustomColumnHeader = config.colDef.isCustomColumnHeader;
		self.isCustomColumnHeader = ko.computed(function () {
			if (self._isCustomColumnHeader == undefined || typeof self._isCustomColumnHeader == "object")
				return false;
			else
				return self._isCustomColumnHeader;
		}, self);

		// Column re-ordering
		self._isReorderable = config.colDef.isReorderable;
		self.isReorderable = ko.computed(function () {
			if (self._isReorderable == undefined || typeof self._isReorderable == "object")
				return true;
			else
				return self._isReorderable;
		}, self);
		...
		self.toggleVisible = function (val) {
			...
			grid.configureColumnWidths();
			window.kg.domUtilityService.BuildStyles(grid);
			if (grid.enableSaveGridSettings) {
				saveDataForGrid(grid.UIGridID());
			}
		};
		...
		self.sort = function () {
			...
			/* Added/Modified for US6628 : Grid sorting behavior upon first column header click */
			var dir;
			if (!self.sortDirection() && window.kg.sortService.getSortItemType) {
				dir = (window.kg.sortService.getSortItemType(self.field, grid.sortedData) || DESC);
			}
			if (!dir) {
	...
	window.kg.EventProvider = function (grid) {
		...
		self.setDraggables = function () {
			if (!grid.config.jqueryUIDraggable) {
				...
				//returns details of column to be replaced
				var calculatehelperPos = function (kogridContainer, ui) {
					var kogridPos = { top: kogridContainer[0].getBoundingClientRect().top, left: kogridContainer[0].getBoundingClientRect().left };
					//in case of browser scroll , scroll distance also needs to be added
					kogridPos.top = kogridPos.top + $('body').scrollTop();
					kogridPos.left = kogridPos.left + $('body').scrollLeft();
					//get relative position of the handler w.r.t. grid
					var handlerRelativepos = { top: ui.position.top - kogridPos.top, left: ui.position.left - kogridPos.left };
					var totalWidth = 0;
					var relativePosleft = 0;
					var idx = grid.visibleColumns().length;
					var arrowPos = 0;
					var kogridViewPort = $(kogridContainer[0]).find(".kgViewport");
					$.each(grid.visibleColumns(), function (index, column) {
						var relativePosright = totalWidth + column.width / 2;
						//checks the closest column with respect to helper
						if ((handlerRelativepos.left + $(kogridViewPort[0]).scrollLeft()) >= relativePosleft && (handlerRelativepos.left + $(kogridViewPort[0]).scrollLeft()) <= relativePosright) {
							idx = index;
							arrowPos = totalWidth;
						}
						relativePosleft = totalWidth + column.width / 2;
						totalWidth += column.width;
					});
					//if the helper is on the last column
					if (idx == grid.visibleColumns().length) {
						arrowPos = totalWidth;
					}
					return { idx: idx, arrowPos: arrowPos };
				}
				...
				grid.$root.find('.kgHeaderSortColumn').draggable({
					...
					start: function (event, ui) {
						self.onHeaderMouseDown(event, ui);
					},
					drag: function (event, ui) {
						//US7635
						$("#kgmove_image_display_top").css("display", "none");
						$("#kgmove_image_display_bottom").css("display", "none");
						var e = event.originalEvent.target;//works for all browser
						var kogridContainer = $(e).closest('.koGrid');

						//check if the helper is within the koGrid area
						if (!kogridContainer[0]) {
							ui.helper.find("span img").attr('src', 'content/images/drop-no.gif');
							return true;
						}
						...
						var idx = calculatehelperPos(kogridContainer, ui).idx;
						var arrowPos = calculatehelperPos(kogridContainer, ui).arrowPos;
						var kogridViewPort = $(kogridContainer[0]).find(".kgViewport");

						var colToReplace = grid.visibleColumns()[idx] ? grid.visibleColumns()[idx] : { isReorderable: ko.observable(grid.visibleColumns()[idx - 1].isReorderable()) };

						// If we have the same column,or the coltomove or coltoreplace is non reorder-able do nothing.
						if (self.colToMove.col == colToReplace || !self.colToMove.col.isReorderable() || (!colToReplace.isReorderable() && idx != grid.visibleColumns().length - 1) || self.colToMove.col.index == colToReplace.index - 1) {
							ui.helper.find("span img").attr('src', 'content/images/drop-no.gif');
							return true;
						}

						//to prevent arrow to show outside the grid if there is a scroll in the grid
						if ((kogridPos.left + arrowPos - kogridViewPort[0].scrollLeft) < kogridPos.left) {
							ui.helper.find("span img").attr('src', 'content/images/drop-no.gif');
							return true;
						}

						ui.helper.find("span img").attr('src', 'content/images/drop-yes.gif');

						if (!$("body").find('#kgmove_image_display_top')[0]) {
							var arrowimgup = $("<img id='kgmove_image_display_top' src='content/images/col-move-top.gif' style='z-index:1000;position:absolute;top:" + (kogridPos.top - 5) + "px;left:" + (kogridPos.left + arrowPos - 4 - kogridViewPort[0].scrollLeft) + "px'>");
							var arrowimgdwn = $("<img id='kgmove_image_display_bottom' src='content/images/col-move-bottom.gif' style='z-index:1000;position:absolute;top:" + (kogridPos.top - 5 + 60) + "px;left:" + (kogridPos.left + arrowPos - 4 - kogridViewPort[0].scrollLeft) + "px'>");
							arrowimgup.appendTo('body');
							arrowimgdwn.appendTo('body');
						}

						$("#kgmove_image_display_top").css({
							top: (kogridPos.top - 5),
							left: (kogridPos.left + arrowPos - 4 - kogridViewPort[0].scrollLeft),
							display: "block"
						});

						$("#kgmove_image_display_bottom").css({
							top: (kogridPos.top - 5 + $(".kgHeaderScroller").height()),
							left: (kogridPos.left + arrowPos - 4 - kogridViewPort[0].scrollLeft),
							display: "block"
						});
					...
					stop: function (event, ui) {
						...
						if (ui.helper.find("span img").attr('src') == 'content/images/drop-no.gif')
							return true;

						var idx = calculatehelperPos(kogridContainer, ui).idx; // returns the index of the column to be replaced

						var colToReplace = grid.visibleColumns()[idx] ? grid.visibleColumns()[idx] : { isReorderable: ko.observable(grid.visibleColumns()[idx - 1].isReorderable()), index: idx };

						// If we have the same column, do nothing.
						if (!colToReplace || (!colToReplace.isReorderable() && idx != grid.visibleColumns().length - 1) || self.colToMove.col == colToReplace) {
							return true;
						}
						// Splice the columns
						...
						if (self.colToMove.col.index < colToReplace.index)
							cols.splice(colToReplace.index - 1, 0, self.colToMove.col);//when column moved from left to right

						else
							cols.splice(colToReplace.index, 0, self.colToMove.col);//when column moved from right to left
		...
		//Header functions
		self.onHeaderMouseDown = function (event, ui) {
			...
			//US7635: Add CSS style to the drag helper div
			if (ui && ui.helper) {
				ui.helper.width($(ui.helper.context).width());
				ui.helper.find(".kgHeaderText").width($(ui.helper.context).find(".kgHeaderText").width());
				ui.helper.addClass("kgHeaderDragHelper");
				ui.helper.css("z-index", 1000);
				ui.helper.find(".kgHeaderText").css("padding-left", "2px");
				ui.helper.find(".kgHeaderText").css("padding-right", "18px");
				ui.helper.prepend("<span style='float: left;padding-top:5px;' ><img src='content/images/drop-no.gif' />");
			}
			//US7635: END
		...
		self.onHeaderDrop = function (event, ui) {
			// checking whether selected column available and is reorderable or not.
			if (!self.colToMove || !self.colToMove.col.isReorderable()) {
				return true;
			}
			// Get the closest header to where we dropped
			var headerContainer = $(event.target).closest('.kgHeaderSortColumn');
			...
			// If we have the same column, do nothing.
			if (!headerScope || !headerScope.isReorderable() || self.colToMove.col == headerScope) {
				return true;
			}
			// Splice the columns
			var cols = grid.columns.peek();
	...
	***********************************************/
	window.kg.RowFactory = function (grid) {
		...
		self.filteredDataChanged = function () {
			...
			self.renderedRange.bottomRow = grid.minRowsToRender();
	...
	***********************************************/
	window.kg.Grid = function (options) {
		var defaults = {
			...
			displaySelectionCheckbox: false, //toggles whether row selection check boxes appear
			selectWithCheckboxOnly: false,
			useExternalSorting: false,
			sortInfo: ko.observable(undefined), // similar to filterInfo
			triggerRestore: ko.observable(undefined),
			multiSelect: true,
			tabIndex: -1,
			...
			showColumnMenu: true,
			showFilter: true,
			resetGridSettingsVisibility: false,
			showPageSize: true,
			enableSaveGridSettings: true,
			...
			removeCellBorder: false, // to remove the border of cell in grid
			enablePaginationBlocks: false,	//US6386
			enableSelectiveDisplay: true,	//US6386
			//enableBackwardPaginationBlocks:false,
			emptyDataText: options.emptyGridMessage ? options.emptyGridMessage : "No data found.",	// US11264, Making Empty grid message configurable.
			viewPortOptions: {
				isfixed: ko.observable(false),
				minRows: ko.observable(5),
				maxRows: ko.observable(10)
			},
			isColMenuDisabled: true,
			isHeaderHeightFixed: false,		//US7008--set whether header should be fixed or stacked
			UIGridID: ko.observable(''),	// DE8041 -- GridName for save Grid Setting.
			allowTrackChange: false,
			//US12642: variables to enhance checkboxes funtionality
			configurableCheckboxes: false,
			checkBoxfilterField: undefined,
			valueForCondition: ''
		},
		self = this;
		...
		//self vars
		self.isColMenuDisabled = options.isColMenuDisabled ? options.isColMenuDisabled : false;
		self.enableSelectiveDisplay = true;
		self.config = $.extend(true, defaults, options);
		self.config.columnDefs = ko.utils.unwrapObservable(options.columnDefs);
		if (self.config.allowTrackChange) {
			self.config.data.extend({ trackChange: true });
		}
		...
		self.triggerRestore = self.config.triggerRestore;
		self.sortedData = self.config.data;
		self.lateBindColumns = false;
		self.filteredData = ko.observableArray([]);
		self.lastSortedColumn = undefined;
		self.showFilter = self.config.showFilter;
		self.resetGridSettingsVisibility = self.config.resetGridSettingsVisibility;
		self.removeCellBorder = self.config.removeCellBorder;
		self.showPageSize = self.config.showPageSize;
		self.enableSaveGridSettings = self.config.enableSaveGridSettings;
		...
		self.UIGridID = self.config.UIGridID; // DE8041 -- GridName for save Grid Setting.
		...
		self.setRenderedRows = function (newRows) {
			...
			if (self.$root != null)
				self.elementDims.rootMaxW = self.$root.width();
			if (self.$root != null)
				self.elementDims.rootMaxH = self.$root.height();
		...
		self.buildColumns = function () {
			...
			//US12642
			if (self.config.configurableCheckboxes === false) {
				if (self.config.displaySelectionCheckbox && self.config.canSelectRows) {
					if (columnDefs.length > 0 && columnDefs[0].field != '\u2714') {
						columnDefs.splice(0, 0, {
							...
							sortable: false,
							resizable: false,
							headerCellTemplate: '<input class="kgSelectionHeader" type="checkbox" data-bind="visible: $grid.multiSelect, checked: $grid.allSelected, click: $grid.toggleallSelected"/>',
			...
			//US12642
			else if (self.config.configurableCheckboxes === true) {
				if (self.config.displaySelectionCheckbox && self.config.canSelectRows) {
					if (self.config.checkBoxfilterField) {
						if (columnDefs.length > 0 && columnDefs[0].field != self.config.checkBoxfilterField) {
							columnDefs.splice(0, 0, {
								field: self.config.checkBoxfilterField,
								width: self.elementDims.rowSelectedCellW,
								sortable: false,
								resizable: false,
								headerCellTemplate: '<input class="kgSelectionHeader" type="checkbox" data-bind="visible: $grid.multiSelect, checked: $grid.allSelected, click: $grid.toggleallVisibleSelected"/>',
								cellTemplate: '<div class="kgSelectionCell"><input class="kgSelectionCheckbox" type="checkbox" data-bind="checked: $parent.selected, visible: $data.getProperty($parent) == \'' + self.config.valueForCondition.toString() + '\' ? false:true, css:{kgCheckBoxClickable: ($data.getProperty($parent) !== \'' + self.config.valueForCondition.toString() + '\')}" /></div>'
							});
						}
					}
				}
			...
			if (columnDefs.length > 0) {
				$.each(columnDefs, function (i, colDef) {
					...
					column.sortDirection(colDef.sortDirection);
					cols.push(column);
		...
		self.configureColumnWidths = function () {
			...
			$.each(cols, function (i, col) {
				...
				if (isNaN(t) && columns[i].visible() == true) {
					t = col.width;
					...
					//condition added to calculate the total width of only the visible columns
					//so the width for columns with variable length gets correctly calculated
					if (!isNaN(t) && columns[i].visible() == true)
			...
			if (asterisksArray.length > 0) {
				...
				if (asteriskVal <= 25)
					asteriskVal = 25;
				// DE6387:calculate the extra weight which is not used to calculate column width and causes white space at the end.
				var remainingPx = remainingWidth % asteriskNum > 0 ? remainingWidth % asteriskNum : 0;
				...
				$.each(asterisksArray, function (i, col) {
					...
					if (i + 1 == asterisksArray.length) {
						...
						// DE6387:adding the extra weight to the last column.
						columns[col.index].width += remainingPx;
					}
		...
		self.init = function () {
			...
			self.filteredData.subscribe(function () {
				...
				//change master flag to true if all the filtered data is selected
				var noOfSelectedData = 0;
				for (var i = 0; i < self.filteredData().length; i++) {
					if (self.filteredData()[i][SELECTED_PROP] !== undefined) {
						if (self.filteredData()[i][SELECTED_PROP] == true)
							noOfSelectedData++;
					}
				}
				//US12642
				if ((self.filteredData().length == (noOfSelectedData) || jQuery('.kgCheckBoxClickable').length == noOfSelectedData) && noOfSelectedData > 0)
		...
		self.resizeOnData = function (col) {
			...
			//str.replace("", "") is used to resolve the issue when the gripper is double clicked//
			col.width = longest = Math.min(Number(col.maxWidth.replace('px', '')), longest + 7); // + 7 px to make it look decent.
			window.kg.domUtilityService.BuildStyles(self);
		};
		self.sortData = function (col, direction) {
			// if external sorting is being used, do nothing.
			self.isSorting = true;
			// To avoid multiple ajax calls for sorted data, new variable, sortInfoLocal,  has been introduced.
			var sortInfoLocal = ko.observable({
				column: col,
				direction: direction
			});
			self.clearSortingData(col);
			if (!self.config.useExternalSorting) {
				self.sortInfo(sortInfoLocal());
				var sortedDataTemp = self.sortedData;
				// if client side sorting then use 'fullDataObject' for sorting, it will have whole data.
				// then slice it to current page
				if (self.config.fullDataObject && self.config.fullDataObject() && self.config.fullDataObject().length) {
					sortedDataTemp = self.config.fullDataObject;
				}
				window.kg.sortService.Sort(self.sortInfo.peek(), sortedDataTemp);
				if (self.config.fullDataObject && self.config.fullDataObject() && self.config.fullDataObject().length) {
					// slicing the sorted data for the current page.
					self.sortedData(self.config.fullDataObject.slice((self.config.pagingOptions.currentPage() - 1) * self.config.pagingOptions.pageSize(), self.config.pagingOptions.currentPage() * self.config.pagingOptions.pageSize()));
				}
			} else {
				self.config.sortInfo(sortInfoLocal.peek());
			}
			self.lastSortedColumn = col;
			if (self.enableSaveGridSettings) {
				saveDataForGrid(self.UIGridID());
			}
			self.isSorting = false;
		};
		self.resetGridSettings = function () {
			if (self.triggerRestore() === true)
				self.triggerRestore(false);
			else
				self.triggerRestore(true);
			if (self.enableSaveGridSettings) {
				saveDataForGrid(self.UIGridID());
			}
		};
		self.clearSortingData = function (col) {
			if (!col) {
				...
				// get the data which is sorted by server
				var colSort = $.grep(self.columns(), function (colI) {
					return colI.sortDirection() !== '' && colI.sortable() === true && colI !== col;
				});

				if (colSort && colSort.length > 0) {
					colSort[0].sortDirection("");
				}
			}
		};
		...
		self.enableSelectiveDisplay = self.config.enableSelectiveDisplay;

		//Paging
		self.enablePaging = self.config.enablePaging;
		self.pagingOptions = self.config.pagingOptions;
		////US6386
		self.enablePaginationBlocks = ko.computed(function () {
			if (self.config.pagingOptions.totalServerItems() > self.config.pagingOptions.pageSize())
				return true;
		});
		//Templates
		...
		self.toggleShowMenu = function () {
			if (self.isColMenuDisabled)
				self.showMenu(false);
			else
				self.showMenu(!self.showMenu());
		};

		self.showMenu.subscribe(function (newValue) {
			// Method to hide Columns menu when user clicks outside the element/container.
			var hideMenu = function (obj) {
				if ($(obj.target).closest('.kgColMenu') && $(obj.target).closest('.kgColMenu').length > 0) {
					return;
				}
				self.showMenu(false);
			};
			...
			if (newValue) {
				// registering the hidemenu only on when the menu is opened.
				$(document).on('mousedown', $.proxy(hideMenu, self));
			}
			else {
				// de-registering the hidemenu on when the menu is closed.
				$(document).off('mousedown', hideMenu);
			}
		});

		$("#reportrange").on("click", function (obj) {
			self.showMenu(false);
		});
		$("#VerticalOptionButtonGroup").on("click", function (obj) {
			self.showMenu(false);
		});
		...
		window.kg.toggleSelection.subscribe(function (_newValue) {
			if (_newValue != null) {
				self.allSelected(_newValue)
				self.selectionService.toggleSelectAll(self.allSelected());
				self.config.afterSelectionChange(self.selectedItems.peek(), this);
				window.kg.toggleSelection(null);
			}
		});

		self.toggleallSelected = function (data, event) {
		...
		//US12642 New function to toggle the selection of the checkboxes(based on visibility of the checkboxes)
		self.toggleallVisibleSelected = function (data, event) {
			if (self.config.beforeSelectionChange(self.sortedData.peek(), this)) {
				self.selectionService.toggleSelectAllVisibleCheckboxes(self.allSelected());
				self.config.afterSelectionChange(self.selectedItems.peek(), this);
			}
			return true;
		}
		...
		self.topPanelHeight = ko.observable(self.config.showGroupPanel === true ? (self.config.headerRowHeight * 2) + 1 : self.config.headerRowHeight);
		self.viewportDimHeight = ko.computed(function () {
			//initial code
			var minHeight = self.rootDim.outerHeight() - self.topPanelHeight() - self.config.footerRowHeight - 2;
			//fix--start
			//fixed issue for the extra space at the end of the rows.
			// calculating the exact height of the view port area based on the total records displayed
			var maxview = 0;
			if (!ko.unwrap(self.config.viewPortOptions.isfixed)) {
				if (self.filteredData().length >= ko.unwrap(self.config.viewPortOptions.minRows))
					maxview = self.filteredData().length * self.config.rowHeight;
				if (self.filteredData().length < ko.unwrap(self.config.viewPortOptions.minRows))
					maxview = ko.unwrap(self.config.viewPortOptions.minRows) * self.config.rowHeight;
			}
			else {
				maxview = ko.unwrap(self.config.viewPortOptions.maxRows) * self.config.rowHeight;//US7817
			}
			//if records are 0 then set a min height.
			if (maxview === 0)
				maxview = ko.unwrap(self.config.viewPortOptions.minRows) * self.config.rowHeight;
			var offset = 0;
			if (self.$viewport !== undefined && self.$viewport !== null) {
				if (self.totalRowWidth() > self.rootDim.outerWidth()) {
					//compensate for scrollbar
					offset += window.kg.domUtilityService.ScrollW;
				}
			}
			//fix--end
			return Math.max(0, maxview + offset);
		});
		...
		self.pageForward = function () {
			/*
			* DE12009: Added number type for variable name 'page'.
			* Reason: value coming as string
			* Fix: convert string to number
			*/
			var page = Number(self.config.pagingOptions.currentPage());
		...
		// Added for client side search/filter functionality[US6476].
		// If current page number is higher than max page then set the current page as max page size.
		// this will applied only on client side search/filter.
		self.maxPages.subscribe(function (newValue) {
			if (newValue) {
				if (!self.config.filterOptions.useExternalFilter && self.filterText() && self.filterText().length > 0) {
					var cPageNumber = self.config.pagingOptions.currentPage();
					if (cPageNumber > newValue) {
						self.config.pagingOptions.currentPage(newValue);
					}
				}
			}
		});
		self.pagingOptions.pageSize.subscribe(function (newValue) {
			if (self.removeCellBorder) {
				$('.noLeftBorder').parent().css('border-left', '0px');
				$('.noRightBorder').parent().css('border-right', '0px');
			}
		});
	...
	***********************************************/
	window.kg.SearchProvider = function (grid) {
		...
		self.resetGridSettingsVisibility = grid.resetGridSettingsVisibility;
		self.removeCellBorder = grid.removeCellBorder;
		self.showPageSize = grid.showPageSize;
		self.enableSaveGridSettings = grid.enableSaveGridSettings;
		self.filterText = grid.config.filterOptions.filterText;
		self.throttle = grid.config.filterOptions.filterThrottle;
		self.fieldMap = {};
		paginationOptions = grid.config.pagingOptions;
		self.evalFilter = function () {
			var filteredDataTemp = ko.observableArray([]);
			var sortedDataTemp = grid.sortedData;
			// use full data object for filter when ko grid is setuped for internal filter.
			if (!self.extFilter && grid.config.fullDataObject && grid.config.fullDataObject() && grid.config.fullDataObject().length) {
				sortedDataTemp = grid.config.fullDataObject;
			}

			if (searchConditions.length === 0) {
				filteredDataTemp(sortedDataTemp.peek().filter(function (item) {
						...
						if (!condition.column) {
							for (var prop in item) {
								if (item.hasOwnProperty(prop)) {
									// get the column setting for the current property
									var isVisibleColumn = ko.utils.arrayFirst(grid.columns(), function (c) {
										return c.field === prop && c._visible() === true;
									});
									// apply search/filter query only for visbiled column
									if (isVisibleColumn) {
			...
			if (!self.extFilter && grid.config.fullDataObject && grid.config.fullDataObject() && grid.config.fullDataObject().length) {
				/*if (searchConditions.length === 0) {*/
				grid.filteredData(filteredDataTemp.slice((paginationOptions.currentPage() - 1) * paginationOptions.pageSize(), paginationOptions.currentPage() * paginationOptions.pageSize()));
				/*} else {
					grid.filteredData(filteredDataTemp.slice(0, paginationOptions.pageSize()));
				}*/
				paginationOptions.totalServerItems(filteredDataTemp().length);
			}
			else {
				grid.filteredData(filteredDataTemp());
			}
	...
	***********************************************/
	window.kg.SelectionService = function (grid) {
		...
		self.ChangeSelection = function (rowItem, evt) {
			...
			//change master flag to true if all the filtered data is selected
			//US12642
			if ((grid.filteredData().length == grid.selectedItemCount() || jQuery('.kgCheckBoxClickable').length == grid.selectedItemCount()) && grid.selectedItemCount() > 0)
		...
		self.toggleSelectAll = function (checkAll) {
			...
			$.each(grid.filteredData(), function (i, item) {
				...
				if (checkAll) {
					...
					var indx = self.selectedItems.indexOf(item);
					if (indx !== -1)
						self.selectedItems.splice(indx, 1);
				}
			});
		...
		// @return - boolean indicating if all items are selected or not
		// @val - boolean indicating whether to select all/de-select all
		self.toggleSelectAllVisibleCheckboxes = function (checkAll) {
			var selectedlength = self.selectedItems().length;
			var fieldName = grid.config.checkBoxfilterField;
			$.each(grid.filteredData(), function (i, item) {
				if (item[fieldName] !== grid.config.valueForCondition.toString()) {
					item[SELECTED_PROP] = checkAll;
					if (checkAll) {
						//DE6567
						if (self.selectedItems.indexOf(item) === -1)
			...
			$.each(self.rowFactory.rowCache, function (i, row) {
				if (row && row.selected && row.entity[fieldName] !== grid.config.valueForCondition.toString()) {
					row.selected(checkAll);
				}
			});
		};
	...
	***********************************************/
	window.kg.StyleProvider = function (grid) {
		...
		grid.headerScrollerStyle = ko.computed(function () {
			return { "min-height": grid.config.headerRowHeight + "px" }; //US7008--changed height to min-height for implementing stacked header
		});
		grid.topPanelStyle = ko.computed(function () {
			return { "width": grid.rootDim.outerWidth() + 2 + "px", "height": grid.topPanelHeight() + "px" };
		});
		grid.headerStyle = ko.computed(function () {
			var hWidth = 0;
			var headerWidth = '';

			hWidth = Math.max(0, grid.rootDim.outerWidth() - window.kg.domUtilityService.ScrollW);
			headerWidth = hWidth && hWidth !== 0 ? hWidth + "px" : "auto";

			return { "width": headerWidth, "min-height": grid.config.headerRowHeight + "px" };//US7008--changed height to min-height for implementing stacked header
		});
		grid.viewportStyle = ko.computed(function () {
			var hWidth = 0;
			var viewerWidth = '';

			hWidth = grid.rootDim.outerWidth();
			viewerWidth = hWidth && hWidth !== 0 ? hWidth + "px" : "auto";

			return { "width": viewerWidth, "height": grid.viewportDimHeight() + "px" };
		});
		grid.footerStyle = ko.computed(function () {
			...
			hWidth = grid.rootDim.outerWidth();
			footerWidth = hWidth && hWidth !== 0 ? hWidth + "px" : "auto";

			return { "width": footerWidth, "height": grid.config.footerRowHeight + "px" };
		});
	};
	...
	***********************************************/
	window.kg.sortService = {
		...
		alphaRE: /[^a-zA-Z0-9]/g, // nasty regex for alphabet parsing
		...
		guessSortFn: function (item) {
			...
			if (item.match(/^-?[£$¤]?[\d,.]+%?$/)) {
				return window.kg.sortService.sortAlphaNumeric;
			}
		...
		sortAlpha: function (a, b) {
			...
			return window.kg.sortService.sortAlphaNumeric(a, b);
		},
		sortAlphaNumeric: function (a, b) {
			var aA = a ? a.toLowerCase().replace(window.kg.sortService.alphaRE, "") : a;
			var bA = b ? b.toLowerCase().replace(window.kg.sortService.alphaRE, "") : b;
			if (aA === bA) {
				var aN = parseInt(a.replace(window.kg.sortService.numberRE, ""), 10);
				var bN = parseInt(b.replace(window.kg.sortService.numberRE, ""), 10);
		...
		sortMMDDStr: function (a, b) {
			var dateA, dateB, mtch, m, d, y;
			mtch = window.kg.sortService.getDateMatch(a);
			if (!mtch) {
				return 1;
			}
			y = mtch[3];
			...
			dateA = y + m + d;
			mtch = window.kg.sortService.getDateMatch(b);
			if (!mtch) {
				return 1;
			}
			y = mtch[3];
		...
		sortData: function (data /*datasource*/, sortInfo) {
			...
			unwrappedData.sort(function (itemA, itemB) {
				...
				// sorting the null/undefined values based on the decided direction.
				if (!propB && !propA) {
					return (direction === ASC) ? (0 - 0) : 0;
				} else if (!propA) {
					return (direction === ASC) ? (0 - 1) : 1;
				} else if (!propB) {
					return (direction === ASC) ? (0 - (-1)) : -1;
				}
		...
		/**
		* Method to get date object array for the given date.
		* @param {string} date input to get the date object array.
		* @returns {array} if the passed value is matched as date then it returns each date part in array, else it returns empty.
		*/
		getDateMatch: function (date) {
			if (!date) {
				return;
			}

			var newDate = new String(date).toString(),
				mtch = newDate.match(window.kg.sortService.dateRE);
			if (!mtch) {
				var newd = new Date(newDate);
				if (newd) {
					newDate = newd.toLocaleDateString();
					mtch = newDate.match(window.kg.sortService.dateRE);
					if (!mtch) {
						return;
					}
	...
	window.kg.domUtilityService = {
		...
		BuildStyles: function (grid) {
			var rowHeight = grid.config.rowHeight,
				...
				sumWidth = 0;
			//US7817
			if (grid.$viewport != null && grid.$viewport != undefined && ko.unwrap(grid.config.viewPortOptions.isfixed) && (ko.unwrap(grid.config.viewPortOptions.maxRows) < grid.filteredData().length))
				grid.$viewport.css("overflow-y", "scroll");
			if (!$style) {
				$style = $('#' + gridId);
				if (!$style[0]) {
			...
			//US7008--to make the grid fixed the below class is added.
			if (grid.config.isHeaderHeightFixed)
				css += "." + gridId + " .kgHeaderFixed {white-space: " + "nowrap}";

			$.each(cols, function (i, col) {
				css += "." + gridId + " .col" + i + " { width: " + col.width + "px; left: " + (sumWidth - 1) + "px; right: " + (trw - sumWidth - col.width) + "px; height: " + rowHeight + "px }" +
					"." + gridId + " .hcol" + i + " { max-width: " + (col.width - 1) + "px;min-width: " + (col.width - 1) + "px;}" + //US7008--seperate class for header column is added and 1px is removed from the header width for border
...
// Functions to save data(Columns) in the database
function saveDataForGrid(gridName) {
	var gridHtmlId = gridName != '' ? gridName : 'reportGrid';
	var gridContainerType = 'div'
	var gridObject = $('#'+gridHtmlId + ' > ' + gridContainerType)[0];
	if (!gridObject || !jQuery('#reportTitle').text()) {
		console.log('Something went wrong while saving the grid');
		return;
	}

	var uiGridObject = ko.dataFor(gridObject);
	var sortInfo = uiGridObject.sortInfo();

	// Filtering the non-reorderable columns.
	var columnIndex = 0;
	var columnList = ko.observableArray();
	ko.utils.arrayForEach(uiGridObject.columns(), function (column) {
		if (column.isReorderable()) {
			columnList.push({
				ColumnName: column.field,
				IsVisible: column.visible(),
				ColumnOrder: columnIndex,
				SortOrder: (sortInfo && sortInfo.column.field == column.field) ? (sortInfo.direction.toString().toLowerCase() == "asc" ? 0 : 1) : null
			});
			columnIndex += 1;
		}
	});
	var newUserGridSetting = {
		GridName: gridName, // jQuery('#reportTitle').text(),
		ColumnNameOrder: columnList()
	};
	saveGridInDB(newUserGridSetting);

	return;
};

function saveGridInDB(koGridDispSetting) {
	var ajax = new Simplex.AjaxConnection(Utils.Constants.AtlasBaseURL);
	ajax.post('Accounting/SetAccountingUserGridSetting', koGridDispSetting)
		.done(function (data) {
		})
		.fail(function (arg) {
			console.log('Something went wrong while saving the grid');
